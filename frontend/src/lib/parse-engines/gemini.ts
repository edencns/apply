...
const GEMINI_SCHEMA_EXTENDED: any = {
  ...
  properties: {
    ...
    // 서류 상세 — 공고마다 20~40건씩 나와 출력량이 가장 큼.
    // 필드를 핵심만 남겨 생성 시간 단축 (evidenceQuote/alternativeDocs/
    // detailedVersion/originalRequired/validityDays 제거).
...
14. \`requiredDocumentsDetailed\`: 각 서류마다 category(공통/신혼부부/생애최초/다자녀가구/노부모부양/기관추천/신생아/일반공급), name, required(필수|해당시|null), issuer(발급처), submitTiming(제출 시점) 만 필수. 그 외는 생략.
