...
const NEWLYWED: ReviewerRole = {
  ...
  ruleChecks: [
    ...
    { key: "income_limit", label: "소득 기준", description: "우선 100%(맞벌이 120%) / 일반 140%(맞벌이 160%) / 추첨 부동산 3.31억 이하. 공고 소득표 대조.", severity: "must", usesFields: ["incomeMonthly", "dualIncome"] },
...
- 소득(2021 도시근로자, 3인이하 기준): 우선 100% 6,208,934원(맞벌이 120% ~7,450,721) / 일반 140% ~8,692,508(맞벌이 160% ~9,934,294) / 추첨: 소득초과+부동산 3.31억 이하.
- 맞벌이 기준소득: 부부 중 1인 소득이 100% 초과하지 않아야 우선. 상위소득: 1인 140% 초과 안 됨.
...
const ELDERLY: ReviewerRole = {
  ...
  ruleChecks: [
    ...
    { key: "elderly_65_3y", label: "만65세 직계존속 3년 부양", description: "만65세 이상 직계존속을 3년 이상 계속 동일 등본 부양. 만60세+ 직계존속 유주택 시 부적격.", severity: "must", usesFields: ["elderlyDependents", "dependYears"] },
...
- 대상: 규칙 제46조. 공고일 현재 강릉/강원 거주, 만65세 이상 직계존속(배우자 직계존속 포함)을 3년 이상 계속 부양(동일 등본 등재)하는 무주택 세대주. 피부양자 배우자도 무주택.
- 무주택기간 산정: 신청자·배우자·피부양자(노부모)·그 배우자 기준. 피부양자가 주택 소유했던 기간은 신청자 무주택기간에서 제외.
- 만60세 이상 직계존속(피부양자 배우자 포함)이 주택 소유 시 유주택자 (다자녀와 다른 점: 60세 기준).
- 청약통장 1순위 자격 필요.
...
const FIRSTLIFE: ReviewerRole = {
  ...
  ruleChecks: [
    ...
    { key: "married_or_child", label: "혼인 또는 자녀 또는 1인가구", description: "혼인 중이거나 미혼 자녀 있음. 1인 가구는 추첨제만(단독세대는 60㎡ 이하).", severity: "must", usesFields: ["marriedOrChild"] },
    { key: "income_limit", label: "소득 기준", description: "우선 130% / 일반 160% / 추첨 부동산 3.31억 이하. 공고 소득표 대조.", severity: "must", usesFields: ["incomeMonthly"] },
  ],
...
- 소득: 우선 130% / 일반 160% / 추첨: 소득초과+부동산 3.31억 이하.
- 1인 가구: 추첨제만 가능. 단독세대는 전용 60㎡ 이하만.
- 비투기/비청약과열이라 일반공급 1순위 조건(2년·세대주·5년내 무당첨) 불필요.
- 선정: 우선 50% → 일반 20% → 추첨 30%(1인가구·자산기준 충족자 포함).
- 세대주 요건 X, 소득·자산 O.`,
};
