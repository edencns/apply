...
function InfoPopover({ label, children }: { label?: string; children: React.ReactNode }) {
  return (
    <span className="relative inline-block group align-middle">
      <button
        type="button"
        className="inline-flex items-center justify-center w-4 h-4 rounded-full bg-accent-soft text-accent text-[10px] font-bold ml-1 hover:bg-surface2 focus:outline-none focus:ring-1 focus:ring-accent"
      ...
      {docs.map((d, i) => (
        <div
          ...
          className={`p-2 rounded border ${
            d.required ? "border-accent-line bg-accent-soft" : "border-amber-200 bg-amber-50/40"
          }`}
        >
          <div className="flex items-center gap-1.5 flex-wrap">
            ...
            {d.required ? (
              <span className="text-[9.5px] bg-accent-soft text-accent px-1 py-0.5 rounded font-semibold">필수</span>
            ) : (
          ...
          {d.examples && d.examples.length > 0 && (
            <div className="mt-1 ml-4 p-1.5 rounded bg-surface border border-accent-line">
              <div className="text-[9.5px] font-semibold text-accent mb-0.5">
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto">
      ...
      {!selected ? (
        ...
        <>
          {/* 컨텍스트 */}
          <div className="mt-2 mb-3 p-3 rounded-lg bg-accent-soft border border-accent-line text-xs text-ink-2 flex items-start gap-2">
                ...
                return (
                  <button
                    ...
                    className={`px-2.5 py-2 rounded-md text-[11.5px] font-medium border inline-flex items-center justify-center gap-1.5 transition-all ${
                      active
                        ? "bg-accent text-[#0a0a0a] border-accent shadow-sm"
                        : "bg-surface text-ink-2 border-border hover:border-accent/50 hover:text-ink"
                    }`}
                  >
